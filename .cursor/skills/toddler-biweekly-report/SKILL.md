...
description: Builds self-contained cream/soft toddler class biweekly newsletter HTML (托班两周周报) by locking the canonical head and hero header from reference-shell.html, replacing only the biweekly date badge (中国大陆工作日起止), then strictly rendering dynamic sections and tips from supplied copy and a photo folder; uses landscape photo grids (6→3×2, 8→4×2), no image captions; then scripts/generate-long-screenshot.py produces the final long PNG. Use when producing biweekly reports with variable sections, images keyed by filename prefix per section, and a PNG image as the final deliverable.
---
...
**Source of truth:** theme shells under `lib/report/templates/{templateId}/shell.html` (default `cream-soft`; also `ocean-fresh`). Local [reference-shell.html](reference-shell.html) mirrors `cream-soft` for offline skill use.
...
- **Footer:** append [reference-footer.html](reference-footer.html) after the dynamic `tips-section` so the red star bar and document close match the reference.
...
1. Start from [reference-shell.html](reference-shell.html); set `.info-badge` to `biweekly_date_range` (must satisfy **中国大陆工作日** rules above).
2. Insert all `.section`… HTML (with correct backgrounds and grids).
3. Insert the full `.tips-section`… block from `body_content`.
4. Append [reference-footer.html](reference-footer.html).
...
1. After the `.html` is complete and saved (e.g. `toddler-biweekly-report-2026-04-11.html`), run from the repo root:
