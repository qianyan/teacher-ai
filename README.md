...
| Command | Purpose |
| --- | --- |
| `npm run prepare:local` | Validate `.env.local` |
...
- `LLM_API_KEY` or provider-specific key (`OPENAI_API_KEY`, `DASHSCOPE_API_KEY`, `ZHIPUAI_API_KEY`, `MOONSHOT_API_KEY`, `MINIMAX_API_KEY`, or `ANTHROPIC_API_KEY`)
...
The GitHub Actions workflow is defined in [`.github/workflows/deploy.yml`](./.github/workflows/deploy.yml):

```text
validate (lint, test:env, build, ci-db-safety)
  -> deploy-preview
  -> verify-preview (smoke tests)
  -> migrate-db (main push only)
  -> deploy-production (main push only)
```

Required repository secrets and variables are documented in [`docs/deployment.md`](./docs/deployment.md).
